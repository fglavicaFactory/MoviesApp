...
class DatabaseManager {
    ...
    static func isMovieWatched(with id: Int) -> Bool {
        guard let watchedIds = defaults.array(forKey: DatabaseKeys.watched)
            as? [Int] else {return false}
        return watchedIds.contains(id)
    }
    
    static func isMovieFavourited(with id: Int) -> Bool{
        guard let favouriteIds = defaults.array(forKey: DatabaseKeys.favourite)
            as? [Int] else {return false}
        return favouriteIds.contains(id)
    }
    static func watchedMovie(with id: Int){
        guard var watchedIds = defaults.array(forKey: DatabaseKeys.watched)
            as? [Int] else {
    ...
    static func favouritedMovie(with id: Int){
        guard var favouriteIds = defaults.array(forKey: DatabaseKeys.favourite)
            as? [Int] else {defaults.set([id], forKey: DatabaseKeys.favourite)
                return
        }
